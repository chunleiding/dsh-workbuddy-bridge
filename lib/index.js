import { A as LoomySessionStore, B as LOOMY_STREAM_IDLE_TIMEOUT_MS, D as processStartTimeMs, E as isHeartbeatProcessAlive, F as LOOMY_CLIENT_VERSION, I as LOOMY_DISPLAY_NAME, L as LOOMY_PROVIDER, M as defaultSessionPath, N as parseLoomySession, O as FALLBACK_LOOMY_MODELS, P as LOOMY_BASE_URL, R as LOOMY_SESSION_FILENAME, S as loomyHostHeartbeatPath, _ as defaultDesktopAuthPath, a as writeHostHeartbeat$1, b as LOOMY_HOST_HEARTBEAT_FILENAME, c as normalizeCredits, d as FALLBACK_WORKBUDDY_MODELS, f as WorkBuddyCatalog, g as defaultDesktopAuthCandidates, h as WorkBuddyCredentialStore, i as workbuddyHostHeartbeatPath, j as defaultSessionCandidates, k as LoomyCatalog, l as prepareChatBody, m as WORKBUDDY_AUTH_FILE_ENV, n as clearHostHeartbeat, o as WorkBuddyUpstreamClient, p as WORKBUDDY_AUTH_FILENAME, r as readHostHeartbeat, s as classifyUpstreamError, t as WORKBUDDY_HOST_HEARTBEAT_FILENAME, u as regionOf, v as parseWorkBuddyAuth, w as writeHostHeartbeat, x as clearHostHeartbeat$1, y as workbuddyOwnAuthPath, z as LOOMY_SESSION_FILE_ENV } from "./heartbeat-QUBrp1np.js";
...
const Config$2 = z.object({ authFile: z.string().description("WorkBuddy desktop auth file (defaults to the app's own location)") });
/**
...
function applyWorkBuddyPlugin(ctx, config) {
	...
	let current = () => config;
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.settings.installSection(ctx, WORKBUDDY_SETTINGS_NS, Config$2, config, {
			setSource(source) {
				current = source;
			},
			onChange() {
