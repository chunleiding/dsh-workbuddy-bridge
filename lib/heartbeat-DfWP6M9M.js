...
function prepareQoderChatBody(source) {
	...
	const model = typeof document["model"] === "string" && document["model"] !== "" ? document["model"] : QODER_AUTO_MODEL;
	const requestId = randomUUID();
	const sessionId = typeof document["session_id"] === "string" && document["session_id"] !== "" ? document["session_id"] : randomUUID();
	...
	const envelope = {
		...
		business: {
			...
			id: randomUUID(),
			...businessName === void 0 ? {} : { name: businessName },
			begin_at: Date.now(),
			stage: "start"
		},
		messages
	...
	if (parameters !== void 0) envelope["parameters"] = parameters;
	if (Array.isArray(document["tools"])) envelope["tools"] = document["tools"];
	if (document["tool_choice"] !== void 0) envelope["tool_choice"] = document["tool_choice"];
...
function parametersOf(document) {
	const parameters = {};
	const maxTokens = document["max_completion_tokens"] ?? document["max_tokens"];
	if (typeof maxTokens === "number" && Number.isFinite(maxTokens)) parameters["max_tokens"] = maxTokens;
	for (const field of SCALAR_PARAMETER_FIELDS) {
		const value = document[field];
		if (typeof value === "number" || typeof value === "string" || Array.isArray(value) && value.every((item) => typeof item === "string")) parameters[field] = value;
	}
	return Object.keys(parameters).length === 0 ? void 0 : parameters;
}
/**
...
*/
function businessNameOf(messages) {
	for (const message of messages) {
		if (typeof message !== "object" || message === null || Array.isArray(message)) continue;
		const wrapped = message;
...
*/
function envelopeErrorDetail(body) {
	if (typeof body !== "string") return void 0;
	const inner = body.trim();
	if (inner === "" || inner === "[DONE]") return void 0;
	let parsed;
	try {
		parsed = JSON.parse(inner);
	} catch {
		return inner.slice(0, ERROR_BODY_LIMIT$1);
	}
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return void 0;
	...
	return typeof code !== "undefined" && String(code) !== "" ? `code ${String(code)}` : void 0;
}
/** Build the error message for an envelope whose `statusCodeValue` failed. */
function envelopeErrorMessage(status, document) {
		...
		if (typeof document["body"] === "string") {
			...
			if (typeof status === "number" && status !== 200) {
				...
				return sseFrame(JSON.stringify({ error: {
					message: envelopeErrorMessage(status, document),
					type: "server"
				} })) + sseFrame("[DONE]");
			}
