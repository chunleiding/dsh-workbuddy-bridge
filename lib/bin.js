#!/usr/bin/env node
import { S as workbuddyOwnAuthPath, i as workbuddyHostHeartbeatPath, l as WorkBuddyUpstreamClient, m as FALLBACK_WORKBUDDY_MODELS, o as BRIDGE_VERSION, r as readHostHeartbeat, s as isHeartbeatProcessAlive, v as WorkBuddyCredentialStore } from "./heartbeat-CH8IWOfb.js";
import { realpathSync } from "node:fs";
import { fileURLToPath } from "node:url";
//#region src/drivers/workbuddy/bin.ts
...
async function doctor(jsonOutput) {
	...
	if (jsonOutput) printJson(report);
		...
		`Host bundle: ${hostAlive ? "running (pid " + String(heartbeat.pid) + ")" : heartbeat !== void 0 ? "stale heartbeat (process exited)" : "not started"}`,
...
async function status(jsonOutput) {
	...
	if (authStatus.state !== "signed-in") {
		...
		else process.stdout.write(`DSH LLM Bridge (workbuddy): signed out\nHost bundle: ${hostState}\n`);
		return 1;
	}
	let credits;
	try {
	...
	process.stdout.write([
		`DSH LLM Bridge (workbuddy): signed in${authStatus.nickname === void 0 ? "" : ` as ${authStatus.nickname}`}`,
