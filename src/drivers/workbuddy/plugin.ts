/**
 * WorkBuddy driver DSH plugin: compose the core mechanisms with the
 * WorkBuddy driver pieces and register the `workbuddy` provider. Streaming,
 * tool calls, compaction, and permissions stay Harness-owned.
 *
 * @module dsh-llm-bridge/drivers/workbuddy/plugin
 */
...
import { createWorkBuddyAdapter, WORKBUDDY_PROVIDER, WORKBUDDY_DISPLAY_NAME } from './adapter.ts'
 ...
 */
export function applyWorkBuddyPlugin(ctx: Context, config: Config): void {
    ...
    .then(() => {
      ...
      void (async () => {
        try {
          ...
          ctx.logger.warn(
            'dsh-llm-bridge: dynamic model catalog unavailable; serving the static fallback list',
            error,
          )
        }
      })()
