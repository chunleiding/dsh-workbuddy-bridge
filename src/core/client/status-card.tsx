...
import type { DriverWebStatus } from '../status-types.ts'

/** Localized copy keys every driver locale must provide. */
export interface DriverCardCopy {
  title: string
  intro: string
  expand: string
  collapse: string
  ...
  creditPackageUnknownSize: string
  creditsError: string
  refresh: string
  refreshing: string
  ...
  freeModel: string
  badgeLimitedFree: string
  badgeNightDiscount: string
  rate: string
}

/** Translation function shape the locale slot provides. */
export type CardT = (key: keyof DriverCardCopy, params?: Record<string, unknown>) => string

/** Props delivered by the Plugin configuration item slot. */
export interface DriverStatusCardProps {
  t: CardT
  /** Same-origin status route this card polls. */
  statusPath: string
  /** Map a platform badge spelling to the localized label; identity by default. */
...
function dotStyle(status: DriverWebStatus['status']): CSSProperties {
...
function ModelOfferRow({ model, t, localizeBadge }: {
  model: NonNullable<Extract<DriverWebStatus, { status: 'signed-in' }>['models']>[number]
  t: CardT
  localizeBadge: (badge: string) => string
}): React.ReactNode {
  return (
      ...
      <div style={quotaLabelStyle}>
        ...
        <span style={modelBadgeStyle}>
          {model.badges?.map(badge => (
            <span key={badge} style={modelBadgeChipStyle}>{localizeBadge(badge)}</span>
          ))}
...
export function DriverStatusCard({ t, statusPath, localizeBadge }: DriverStatusCardProps): React.ReactNode {
  const [open, setOpen] = useState(false)
  const [status, setStatus] = useState<DriverWebStatus>({ status: 'signed-out' })
  const [busy, setBusy] = useState(false)
  const mounted = useRef(true)
  const badgeLocalizer = localizeBadge ?? ((badge: string) => badge)
  ...
  const refresh = useCallback(async (signal?: AbortSignal): Promise<void> => {
    try {
      ...
      if (mounted.current && signal?.aborted !== true) setStatus(value as DriverWebStatus)
    } catch (error: unknown) {
            ...
            {status.status === 'signed-in'
              ? <>
                  ...
                  {status.updatedAt === undefined ? null
                    : <p style={bodyStyle}>{t('sessionUpdatedAt', { time: formatTime(status.updatedAt) })}</p>}
                  ...
                  {status.models === undefined || status.models.length === 0 ? null : (
                    <div style={quotaListStyle}>
                      ...
                      {status.models.map(model => (
                        <ModelOfferRow key={model.id} model={model} t={t} localizeBadge={badgeLocalizer} />
                      ))}
                    </div>
                  )}
                </>
